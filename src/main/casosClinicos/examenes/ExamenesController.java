...
public class ExamenesController {
    ...
    public void initManager(CasosManager casosManager, String text) {
        ...
        guardar.setOnAction(event -> {
            guardarExamenes(casosManager, text);
            Stage stage = (Stage) guardar.getScene().getWindow();
            stage.close();
        });
    }

    private void guardarExamenes(CasosManager casosManager, String text){
        ...
        casosManager.guardarExamen(text, nombres, costos);
    }

    public void initManager(CasosManager casosManager, String text, List<JSONObject> examenes) {
        List<String> nombres = new LinkedList<>();
        List<Integer> costos = new LinkedList<>();
        ...
        for (int i = 0; i < examenes.size(); i++){
            reNombre.get(i).setText((String) examenes.get(i).get("nombre"));
            reCosto.get(i).setText(String.valueOf(((Long) examenes.get(i).get("costo")).intValue()));
        }
        ...
        guardar.setOnAction(event -> {
            guardarReExamenes(casosManager, text);
            Stage stage = (Stage) guardar.getScene().getWindow();
            stage.close();
        });
    }

    private void guardarReExamenes(CasosManager casosManager, String text) {
        List<String> nombres = new LinkedList<>();
        List<Integer> costos = new LinkedList<>();
        ...
        casosManager.guardarReExamenes(text, nombres, costos);
    }
